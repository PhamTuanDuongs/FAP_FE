...
import { Button, Table, Thead, Tbody, Tr, Th, Td, Box, Text } from '@chakra-ui/react';
...
const InstructorSchedule: React.FC = () => {
    ...
    const instructorIdNumber = parseInt(instructorId || '0', 10);
    ...
    useEffect(() => {
        const fetchSchedules = async () => {
            try {
                const data = await GetInstructorSchedulesAPI(instructorIdNumber);
    ...
    const handleTakeAttendance = (scheduleId: number) => {
        navigate(`/UpdateAttendance/${instructorIdNumber}/${scheduleId}`);
    };
    ...
    return (
        <SidebarWithHeader>
            <Box p={5}>
                <Text fontSize="2xl" fontWeight="bold" mb={4}>Take Attendance</Text>
                                ...
                                <Td>
                                    {schedule.status ? (
                                        <Button
                                            mt={2}
                                            colorScheme="blue"
                                            onClick={() => navigate(`/AttendanceList/${schedule.id}/${instructorId}`)}
                                        >
                                            View Attendance
                                        </Button>
                                    ) : (

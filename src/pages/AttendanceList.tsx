...
import { Table, Thead, Tbody, Tr, Th, Td, Box, Text, Spinner, Alert, AlertIcon, Button } from "@chakra-ui/react";
...
const AttendanceList: React.FC = () => {
    const { scheduleId, instructorId } = useParams<{ scheduleId: string; instructorId: string }>();
    ...
    useEffect(() => {
        const fetchAttendances = async () => {
            try {
                const attendances: AttendanceResponse[] = await GetAttendancesByScheduleAPI(Number(scheduleId), Number(instructorId));
    ...
    return (
        <SidebarWithHeader>
            <Box p={4}>
                <Text fontSize="2xl" mb={4}>Attendance List</Text>
                ...
                <Button
                    colorScheme="blue"
                    mt={4}
                    onClick={() => navigate(`/UpdateAttendance/${instructorId}/${scheduleId}`)}
                >
                    Edit Attendance
                </Button>
            </Box>

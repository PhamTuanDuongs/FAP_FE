...
import { GetSubjectByIdAPI, UpdateSubjectAPI } from "../services/Subject";
...
function UpdateSubject() {
  ...
  const formik = useFormik({
    ...
    const response = UpdateSubjectAPI(Number(params.id),newSubjectData);
                response.then((res) => {
  ...
  useEffect(() => {
    const fetchSubject = async () => {
      const response = await GetSubjectByIdAPI(Number(params.id));
      setSubject(response);
      formik.setValues({
        code: response.code,
        name: response.name,
        slots: response.manageSlot,
      });
    };
      ...
      <Container>
        <form onSubmit={formik.handleSubmit}>
          ...
          <Input
            ...
            onChange={formik.handleChange}
            onBlur={formik.handleBlur}
            isInvalid={formik.touched.slots && Boolean(formik.errors.slots)}
          ></Input>
          {formik.errors.slots && (

...
import { Table, TableContainer, Tbody, Td, Th, Thead, Tr, Alert, AlertIcon, Box, Heading, Text, Divider, Select, HStack, FormControl, FormLabel } from "@chakra-ui/react";
...
const StudentAttendanceDetails: React.FC = () => {
    ...
    const [attendanceDetails, setAttendanceDetails] = useState<StudentAttendanceDetailResponse[]>([]);
    const [filteredDetails, setFilteredDetails] = useState<StudentAttendanceDetailResponse[]>([]);
    ...
    useEffect(() => {
        if (studentId) {
            const fetchAttendanceDetails = async () => {
                try {
                    const data = await GetStudentAttendanceDetailsAPI(parseInt(studentId));
                    setAttendanceDetails(data);


                    const uniqueGroups = Array.from(new Set(data.map(detail => detail.courseName))).sort();
    ...
    useEffect(() => {
        ...
        const filtered = attendanceDetails.filter(detail =>
            (groupFilter === '' || detail.courseName === groupFilter)
        );

        const sortedFilteredDetails = filtered.sort((a, b) => {
            const dateA = new Date(`${a.dateAttended?.split('/').reverse().join('-')}T00:00:00`);
            const dateB = new Date(`${b.dateAttended?.split('/').reverse().join('-')}T00:00:00`);
            return dateA.getTime() - dateB.getTime();
        });
        setFilteredDetails(sortedFilteredDetails);
    }, [groupFilter, attendanceDetails]);

    if (loading) return <Box p={5}><Text>Loading...</Text></Box>;
    if (error) return (
        <SidebarWithHeader>
            <Box p={5}>
                <Alert status="error">
                        ...
                        <Tbody>
                            {filteredDetails.map((detail, index) => (
                                <Tr key={detail.scheduleId}>
                                    ...
                                    <Td color={detail.status === "1" ? 'green.500' : detail.status === "2" ? 'red.500' : 'gray.500'}>
                                        {detail.status === "1"
                                            ? 'Present'
                                            : detail.status === "2"
                                                ? 'Absent'
